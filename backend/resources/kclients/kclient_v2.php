...
class KClickClient
{
    ...
    const STATE_SESSION_KEY = 'tds_state';
    const STATE_SESSION_EXPIRES_KEY = 'tds_state_expires';
    /**
     * @var KHttpClient
     */
    private $_httpClient;
    ...
    private function _saveTdsCookies($uniquenessCookie, $cookies, $ttl)
    {
    ...
    {
        $this->param('force_choose_offer', true);
    }
...
class KHttpClient
{
    ...
    public function request($url, $opts = array())
    {
        ...
        $result = curl_exec($ch);
        if (curl_error($ch)) {
            throw new KClientError(curl_error($ch));
        }
