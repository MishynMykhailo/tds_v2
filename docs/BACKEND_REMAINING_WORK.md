...
## 3. Явные 501-стабы (уже видны в коде, конкретная задача) — 3.2 ЗАКРЫТО, 3.1 частично (реально заблокировано)
...
**Раздел 8 — ЗАКРЫТО (2026-09-03).** Оба пункта из "осталось" ниже
(исторические, из более ранней сессии) выполнены отдельными более
поздними записями: (а) все ~7 stacktrace/Domains-падений закрыты в
разделе 6 ("stacktrace-падения — ЗАКРЫТО", "DomainsTest — ЗАКРЫТО"); (б)
контрактные тесты написаны для ВСЕХ 43 контроллеров (раздел 6,
завершено в этой сессии).

**Важная поправка**: утверждение ниже "~40 контроллеров... расхождений
не найдено" оказалось НЕВЕРНЫМ для многих из них — это была
поверхностная проверка (имена action'ов + базовая форма ответа), а не
живая построчная сверка. Реальное написание контрактных тестов в разделе
6 для этих же "уже проверенных" контроллеров (Reports, Editor, Cleaner,
весь TPI-кластер, Branding и др.) нашло МНОГО реальных расхождений,
которые поверхностный аудит пропустил (неверные fallback-строки,
отсутствующие ACL-гейты, неверные статус-коды, побочные эффекты на
read-only действиях и т.д. — полный список в `docs/PORTING_LOG.md`).
**Урок для будущих сессий**: "проверил имена action'ов, форма ответа
похожа" ≠ "расхождений нет" — только живая curl-сверка каждого
конкретного edge-кейса (404 vs 403, точный текст ошибки, побочные
эффекты) реально ловит баги этого класса.
...
- ~~Остальные ~40 контроллеров проверены живьём (имена action'ов, базовая
  ...
  "проверено — не найдено" в `docs/PORTING_LOG.md`.~~ См. поправку выше.
