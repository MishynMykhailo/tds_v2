...
- `AdminApiController::indexAction` — отдаёт JSON-заглушку
  (`{"message": "..."}`) вместо живой Swagger-UI HTML-страницы легаси.
  Низкий приоритет (страница документации API, ничем не покрыта и не
  используется программно). **Проверено и НЕ баг**: `specAction` УЖЕ
  делает настоящий HTTP 302 на `https://admin-api.docs.tds.io/
  openapi.yaml` (`new RedirectResponse(...)`, подтверждено живым curl —
  `Location`-заголовок, пустое тело, код 302) — фоновый аудит-агент
  ошибочно сообщил про "200 с HTML meta-refresh", живая проверка это не
  подтвердила; не доверять такому отчёту без личной перепроверки (тот
  же принцип, что для докблоков).
